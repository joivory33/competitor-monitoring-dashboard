...
if not st.session_state.api_authenticated:
    ...
    # -----------------------------------------------------------------
    # 💡 [업데이트된 상세 가이드] 네이버 로그인 API 우회 설정 족집게 가이드
    # -----------------------------------------------------------------
    st.markdown("---")
    with st.expander("🔑 [초간단 1분] 네이버 API 발급 및 상세 설정 가이드 (보고 똑같이 따라 하세요!)", expanded=True):
        st.markdown("""
        ### 🚨 중요 공지: 네이버 API 검색 항목 미노출 해결 안내
        현재 네이버 검색 API 발급 정책 변경으로 인해, 기존 개발자 센터 등록 화면 사용 API 목록에서 '검색' 항목이 노출되지 않습니다. 
        대시보드 인증 및 정상 작동을 위해 아래의 **[네이버 로그인] API 설정을 통한 우회 등록 가이드**를 그대로 따라 해주세요!

        ---

        ### 1단계: 애플리케이션 등록 및 API 선택
        1. **[네이버 개발자 센터](https://developers.naver.com/)**에 로그인 후 **[Application] ➡️ [애플리케이션 등록]** 메뉴로 이동합니다.
        2. **애플리케이션 이름**: `검색 키워드` (또는 원하는 이름 입력)
        3. **사용 API**: 드롭다운 목록에서 **`네이버 로그인`**을 선택합니다.

        ### 2단계: 제공 정보 최소 선택 (★이대로만 체크해 보안 심사 패스하기)
        *불필요한 개인정보 항목 수집 시 심사가 지연되거나 반려될 수 있으므로, 대시보드 구동에 필요한 최소 정보만 아래와 같이 체크합니다.*
        * **회원이름**: `필수` 또는 `추가` 선택 (나머지 체크박스는 모두 해제)
        * **성별**: `필수` 또는 `추가` 선택 (나머지 체크박스는 모두 해제)
        * **출생연도**: `필수` 또는 `추가` 선택 (나머지 체크박스는 모두 해제)
        * 💡 *나머지 연락처 이메일 주소, 별명, 프로필 사진, 생일, 연령대, 휴대전화번호 등은 전부 비워두시면(체크 해제) 즉시 키가 발급됩니다.*

        ### 3단계: 로그인 오픈 API 서비스 환경 설정
        1. **로그인 오픈 API 서비스 환경** 드롭다운 메뉴에서 **`PC 웹`**을 선택하여 추가합니다.
        2. 추가된 주소 입력창 두 곳에 현재 대시보드 URL 주소를 동일하게 입력합니다.
        ...
        3. 하단의 등록 완료 버튼을 누릅니다.

        ### 4단계: Client ID & Secret 입력 및 가동
        * 등록 완료 후 생성된 애플리케이션 정보에서 **Client ID**와 **Client Secret**을 복사한 뒤, 대시보드 맨 위의 입력창에 붙여넣고 로그인해 주세요!
        """)
else:
    # 인증 완료 상태 및 로그아웃 버튼
