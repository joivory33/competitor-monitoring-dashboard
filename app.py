...
if not st.session_state.api_authenticated:
    ...
    # -----------------------------------------------------------------
    st.markdown("---")
    with st.expander("🔑 [초간단 1분] 네이버 검색 API 발급 및 상세 설정 가이드 (보고 똑같이 따라 하세요!)", expanded=True):
        st.markdown("""
        ### 🚨 중요 공지: 검색 API 발급처 변경 안내 (2026년 최신)
        기존 **네이버 개발자 센터(Naver Developers)에서는 검색 API 신규 발급이 완전히 중단**되어 사용 API 목록에 '검색'이 나타나지 않습니다. 
        이제부터는 **`네이버 클라우드 플랫폼(NCP)`**을 통해 발급받으셔야 합니다. 아래 순서대로 설정하면 1분 만에 끝납니다!

        ---

        ### 1단계: 네이버 클라우드 콘솔 접속
        1. **[네이버 클라우드 콘솔(NCP)](https://www.ncloud.com/)**에 접속하여 로그인합니다. (네이버 아이디로 로그인 가능)
        2. 로그인 후 우측 상단의 **`[콘솔]`** 버튼을 클릭하여 대시보드로 이동합니다.

        ### 2단계: NAVER API HUB 서비스 선택
        1. 왼쪽 메뉴에서 **`All Services` ➡️ `Application Services` ➡️ `NAVER API HUB`**를 찾아 클릭합니다.
        2. 화면 중앙의 **`+ Application 등록`** 버튼을 누릅니다.

        ### 3단계: 필수 및 선택 항목 세부 설정 (★이대로만 체크하세요!)
        
        #### ① 사용 API 설정
        * **`사용 API`** 목록에서 **`네이버 로그인`**을 체크 선택합니다.
          * *NCP 정책 상, 검색 기능을 정상적으로 사용하려면 로그인 API가 필수적으로 함께 매핑되어야 합니다.*

        #### ② 제공 정보 선택 (화면 상단 표)
        * **[회원이름, 연락처 이메일 주소, 별명, 프로필 사진, 성별, 생일, 연령대, 출생연도, 휴대전화번호]**
        * 🚨 **행동 지침**: 이 표에 등장하는 모든 항목의 **필수/추가 체크박스를 단 하나도 체크하지 않고 '전부 해제(빈칸)'**로 비워둡니다.
        * 💡 *이유: 대시보드는 개인정보를 절대 수집하지 않습니다. 전부 비워두셔야 심사 대기 없이 즉시 키가 발급되어 바로 사용할 수 있습니다.*

        #### ③ 로그인 오픈 API 서비스 환경 설정 (화면 하단)
        1. **`환경 추가`** 드롭다운 박스에서 **`PC 웹`**을 선택해 추가합니다.
        ...
        * 등록이 완료되면 생성된 애플리케이션 우측의 **`[인증키 관리]`** 버튼을 클릭해 **Client ID**와 **Client Secret**을 복사한 뒤, 대시보드 맨 위의 입력창에 붙여넣어 주세요!
        """)
else:
    # 인증 완료 상태 및 로그아웃 버튼
...
# 💡 [완전 제거 완료] 옛날 가이드 코드("API 인증키 발급 가이드" 사이드바 표출 로직)를 여기서 완벽히 지웠습니다.
