...
if not st.session_state.api_authenticated:
    ...
    # -----------------------------------------------------------------
    # 💡 [필독] 1분 만에 끝내는 API 발급 가이드 (최신 버전 반영)
    # -----------------------------------------------------------------
    st.markdown("---")
    st.subheader("🔑 네이버 API 발급 및 등록 가이드")

    with st.expander("📍 [필독] 1분 만에 끝내는 API 발급 가이드 (최신 버전)", expanded=True):
        st.markdown("""
        **⚠️ 중요 공지: 네이버 검색 API 정책 변경**
        현재 네이버 검색 API 발급 창구가 *네이버 클라우드 플랫폼(NCP)*으로 이관되어, 기존 개발자 센터 등록 화면에서는 '검색' API 항목이 노출되지 않습니다.
        
        대시보드의 정상적인 작동(API 키 검증 통과)을 위해, 기존 개발자 센터에서 **[네이버 로그인] API를 활용하여 우회 등록**해주시기 바랍니다. 아래 가이드를 그대로 따라 하시면 1분 안에 완료됩니다.
        
        ---
        
        ### 1️⃣ 애플리케이션 등록 및 API 선택 (첫 번째 단계)
        * **애플리케이션 이름:** `검색 키워드` (또는 자유롭게 입력)
        * **사용 API:** 드롭다운 목록에서 **[네이버 로그인]** 선택
        
        ### 2️⃣ 제공 정보 설정 (두 번째 단계)
        *불필요한 개인정보 수집을 비활성화하여 보안 심사를 생략하고 즉시 발급받기 위해 아래 3가지 항목만 최소 선택합니다.*
        * **회원이름:** `필수` 또는 `추가` 선택 (나머지는 모두 체크 해제)
        * **성별:** `필수` 또는 `추가` 선택 (나머지는 모두 체크 해제)
        * **출생연도:** `필수` 또는 `추가` 선택 (나머지는 모두 체크 해제)
        
        ### 3️⃣ 로그인 오픈 API 서비스 환경 설정 (세 번째 단계)
        * **환경 추가:** 드롭다운 메뉴에서 **[PC 웹]** 선택 후 등록
        * **서비스 URL:** 아래의 대시보드 주소를 그대로 복사해서 입력합니다.
          ```text
          https://instagram-insight-dashboard-yfksdz8sudm8rqyrxy3cqz.streamlit.app/
          ```
        * **네이버 로그인 Callback URL:** 아래의 주소(서비스 URL과 동일)를 입력합니다.
          ```text
          https://instagram-insight-dashboard-yfksdz8sudm8rqyrxy3cqz.streamlit.app/
          ```
          
        ---
        💡 **설정 완료 후:** 발급된 `Client ID`와 `Client Secret`을 대시보드 입력창에 붙여넣고 로그인하시면 바로 모니터링 기능을 사용하실 수 있습니다!
        """)
else:
    # 인증 완료 상태 및 로그아웃 버튼
