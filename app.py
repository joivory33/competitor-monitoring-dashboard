...
if not st.session_state.api_authenticated:
    ...
    # -----------------------------------------------------------------
    st.markdown("---")
    with st.expander("🚨 [필독] 네이버 API 발급 창구 변경 및 1분 발급 가이드 (여기서 보고 똑같이 입력하세요!)", expanded=True):
        st.markdown("""
        ### 📢 중요 공지: 검색 API 발급처 변경 안내 (2026년 최신)
        기존 **네이버 개발자 센터(Naver Developers)에서는 검색 API 발급이 중단**되어 검색창이나 사용 API 목록에 '검색'이 나타나지 않습니다. 
        이제부터는 **`네이버 클라우드 플랫폼(NCP)`**을 통해 발급받으셔야 합니다. 아래를 따라서 진행하시면 아주 간단합니다!

        ---

        ### 1단계: 네이버 클라우드 플랫폼 접속 및 가입
        1. **[네이버 클라우드 콘솔(NCP)](https://www.ncloud.com/)**에 접속하여 로그인합니다. (쓰시던 네이버 아이디로 간편 로그인/가입 가능)
        2. 로그인 후 우측 상단의 **`[콘솔]`** 버튼을 클릭하여 관리 화면으로 이동합니다.

        ### 2단계: NAVER API HUB 메뉴 이동 및 등록 시작
        1. 콘솔 왼쪽의 검색 메뉴에서 **`All Services` ➡️ `Application Services` ➡️ `NAVER API HUB`** 순서대로 메뉴를 클릭해 이동합니다.
        2. 화면에 나타나는 **`+ Application 등록`** 버튼을 클릭합니다.

        ### 3단계: 족집게 설정 값 체크 및 입력 (★실패 없는 설정법)
        
        * **애플리케이션 이름**: 자유롭게 작성 (예: `경쟁사 모니터링 대시보드`)
        
        #### ① 사용 API 선택
        * 목록 드롭다운에서 **`네이버 로그인`**만 체크하여 추가합니다. 
          * 💡 *팁: 네이버 정책상 검색 API를 쓰려면 로그인 API 등록이 필수 매핑 구조로 함께 가야 합니다.*

        #### ② 제공 정보 선택 (화면 중앙 표 - ★이게 제일 중요!)
        * 테이블에 등장하는 **[회원이름, 연락처 이메일 주소, 별명, 프로필 사진, 성별, 생일, 연령대, 출생연도, 휴대전화번호]** 항목들이 있습니다.
        * 🚨 **행동 지침**: 이 모든 항목의 **필수/추가 체크박스를 단 하나도 체크하지 말고 전부 빈칸(해제)**으로 두어야 합니다.
          * 💡 *이유: 저희 대시보드는 개인정보를 수집하지 않고 순수 검색 기능만 사용합니다. 하나라도 체크할 시 까다로운 개인정보 검토 및 보안 심사 대상으로 분류되어 즉시 사용이 보류되거나 차단됩니다.*

        #### ③ 로그인 오픈 API 서비스 환경 설정 (화면 하단)
        1. 드롭다운 박스에서 **`PC 웹`**을 클릭하여 환경을 추가해 줍니다.
        2. 추가된 주소 입력창 두 곳에 아래 대시보드 URL 주소를 동일하게 복사하여 붙여넣습니다.
        ...
        #### ④ 이용 약관 동의 및 완료
        * 맨 하단 이용 약관에 체크하고 최종 **`등록하기`**를 누릅니다.

        ### 4단계: 인증키(Client ID & Secret) 확인 및 입력
        * 완료 후 생성된 서비스 우측의 **`[인증키 관리]`** 버튼을 누릅니다.
        * 발급된 **Client ID**와 **Client Secret** 값을 복사하여 대시보드 맨 상단 입력창에 넣고 로그인하면 완료됩니다!
        """)
else:
    # 인증 완료 상태 및 로그아웃 버튼
